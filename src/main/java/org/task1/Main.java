...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Thread threadLength3 = new Thread(() -> {
            for(int i=0; i < texts.length; i++){
                if(texts[i].length() == 3){
                    if(isPalindrome(texts[i]) || isOneLetter(texts[i]) || isAscendingRow(texts[i])){
        ...
        Thread threadLength4 = new Thread(() -> {
            for(int i=0; i < texts.length; i++){
                if(texts[i].length() == 4){
                    if(isPalindrome(texts[i]) || isOneLetter(texts[i]) || isAscendingRow(texts[i])){
        ...
        Thread threadLength5 = new Thread(() -> {
            for(int i=0; i < texts.length; i++){
                if(texts[i].length() == 5){
                    if(isPalindrome(texts[i]) || isOneLetter(texts[i]) || isAscendingRow(texts[i])){
        ...
        Thread[] threads = new Thread[] {threadLength3, threadLength4, threadLength5};
